...
export class BRPNpcSheet extends foundry.appv1.sheets.ActorSheet {
  ...
  static get defaultOptions() {
    ...
    return foundry.utils.mergeObject(super.defaultOptions, {
      ...
      template: "systems/sla-industries-brp/templates/actor/npc-sheet.html",
      width: 595,
      height: 770,
    });
  }
  ...
  async getData() {
    ...
    context.useBeastiary = game.settings.get('sla-industries-brp', 'beastiary')
    context.statOptions = await BRPSelectLists.addStatOptions("");
    context.showArmour = false

    context.resource = 2
    if (game.settings.get('sla-industries-brp', 'useSAN')) { context.resource++ }
    if (game.settings.get('sla-industries-brp', 'useRes5')) { context.resource++ }

    if (game.settings.get('sla-industries-brp', 'beastiary')) { context.resource = 5 }
  ...
  async _onSkillEdit(event) {
    ...
    if (['base', 'currHP', 'quantity', 'npcVal', 'allegPoints', 'hpCurr', 'ammoCurr', 'ammoReserve'].includes(field)) {
  ...
  async _onAmmoAction(event) {
    ...
    const action = event.currentTarget.dataset.ammoAction;
  ...
  async _selectPower() {
    ...
    const html = await foundry.applications.handlebars.renderTemplate('systems/sla-industries-brp/templates/dialog/selectPower.html', data)
