...
export class BRPActor extends Actor {
  ...
  async _prepareCharacterData(actorData) {
    ...
    const systemData = actorData.system;
    systemData.sla = systemData.sla ?? {};
    if (!Number.isFinite(Number(systemData.sla.credits))) {
      systemData.sla.credits = 0;
    } else {
      systemData.sla.credits = Number(systemData.sla.credits);
    }
    ...
    for (let itm of actorData.items) {
      if (itm.type === 'skillcat') {
        let bonus = 0;
        if (game.settings.get('sla-industries-brp', 'skillBonus') === "1" && itm.system.stat != 'none') {
          bonus = Math.ceil(systemData.stats[itm.system.stat].total / 2);
        } else if (game.settings.get('sla-industries-brp', 'skillBonus') === "2") {
    ...
    const skillCreationCap = SLASkillPoints.getCreationCapForActor(actorData)
    for (let itm of actorData.items) {
      ...
      if (itm.type === 'skill' || itm.type === 'magic' || itm.type === 'psychic') {
        const rawTotal = itm.system.base + itm.system.xp + itm.system.effects + itm.system.profession + itm.system.personality + itm.system.personal + itm.system.culture
        itm.system.total = (systemData.creationMode && itm.type === "skill")
          ? Math.min(skillCreationCap, rawTotal)
          : rawTotal
        ...
        const hitLoc = game.settings.get('sla-industries-brp', 'useHPL') ? actorData.items.get(itm.system.hitlocID) : null
        //Calc encumbrance based on carry status and if using HPL
        if (itm.system.equipStatus === 'carried') {
          if (game.settings.get('sla-industries-brp', 'useHPL') && hitLoc?.system?.fractionENC) {
        ...
        if (itm.system.equipStatus != 'stored') {
          if (itm.system.pSCurr > 0) { systemData.psCurr = systemData.psCurr + itm.system.pSCurr }
          if (itm.system.pSMax > 0) { systemData.psMax = systemData.psMax + itm.system.pSMax }
          if (game.settings.get('sla-industries-brp', 'useHPL') && hitLoc?.system) {
      ...
      } else if (itm.type === 'hit-location' && game.settings.get('sla-industries-brp', 'useHPL')) {
  ...
  async _prepareNpcData(actorData) {
    ...
    for (let [key, stat] of Object.entries(actorData.system.baseStats)) {
      ...
      stat.visible = true;
      if (key === 'edu' && !game.settings.get('sla-industries-brp', 'useEDU')) { stat.visible = false }
    }
  ...
  _prepDerivedStats(actorData) {
    ...
    if (actorData.type === 'character') {
      const hpMod = Number(game.settings.get('sla-industries-brp', 'hpMod') ?? 1);
      const pulpMultiplier = game.settings.get('sla-industries-brp', 'pulpSla') ? 2 : 1;
      systemData.health.max = Math.ceil((systemData.stats.con.total + systemData.stats.siz.total) * hpMod * pulpMultiplier / 2) + systemData.health.mod + (systemData.health.effects ?? 0);
    } else {
      let health = 0
      let formula = ""
      let statCount = 0
    ...
    if (actorData.type === 'character' && !SLAEbbSystem.isEbbSpecies(actorData)) {
      // Flux is SLA EBB-only and only available to Ebon / Brain Waster characters.
      systemData.power.max = 0;
      systemData.power.value = 0;
    } else {
      systemData.power.value = Math.max(0, Math.min(Number(systemData.power.value ?? 0), Number(systemData.power.max ?? 0)));
    }

    const derivedSanMax = Math.max(1, Number(systemData.stats.pow.total ?? 0) * 5);
    const currentSanMax = Number(systemData.sanity?.max ?? 0);
    const currentSan = Number(systemData.sanity?.value ?? 0);
    const needsSanInit = (!Number.isFinite(currentSanMax) || currentSanMax <= 0 || (currentSanMax === 5 && currentSan === 5 && derivedSanMax > 5));
    const needsLegacySanMigration = (Number.isFinite(currentSanMax) && currentSanMax > 0 && currentSanMax < derivedSanMax && currentSanMax <= 10);
    if (needsSanInit || needsLegacySanMigration) {
      // Migrate legacy small SAN pools (e.g. 3/5) to POWx5 while preserving any existing SAN loss.
      const legacyLoss = (Number.isFinite(currentSanMax) && Number.isFinite(currentSan))
        ? Math.max(0, Number(currentSanMax) - Number(currentSan))
        : 0;
      systemData.sanity.max = derivedSanMax;
      systemData.sanity.value = Math.max(0, derivedSanMax - legacyLoss);
    } else {
      systemData.sanity.max = Math.max(1, currentSanMax);
      systemData.sanity.value = Math.max(0, Math.min(Number(currentSan ?? 0), Number(systemData.sanity.max ?? 0)));
    }

    if (game.settings.get('sla-industries-brp', 'useRes5')) {
      const derivedCoolMax = Math.max(1, Number(systemData.stats.pow.total ?? 0) * 2);
      systemData.res5.max = derivedCoolMax;
      if (!Number.isFinite(Number(systemData.res5.value))) {
        systemData.res5.value = derivedCoolMax;
      }
      systemData.res5.value = Math.max(0, Math.min(Number(systemData.res5.value ?? 0), Number(systemData.res5.max ?? 0)));
    }
    ...
    if (game.settings.get('sla-industries-brp', 'ppLabelLong')) {
      systemData.power.label = game.settings.get('sla-industries-brp', 'ppLabelLong')
    } else {
      systemData.power.label = game.i18n.localize('BRP.pp')
    }
    if (game.settings.get('sla-industries-brp', 'ppLabelShort')) {
      systemData.power.labelAbbr = game.settings.get('sla-industries-brp', 'ppLabelShort')
    } else {
      systemData.power.labelAbbr = game.i18n.localize('BRP.ppShort')
    }
    if (game.settings.get('sla-industries-brp', 'hpLabelLong')) {
      systemData.health.label = game.settings.get('sla-industries-brp', 'hpLabelLong')
    } else {
      systemData.health.label = game.i18n.localize('BRP.health')
    }
    if (game.settings.get('sla-industries-brp', 'hpLabelShort')) {
      systemData.health.labelAbbr = game.settings.get('sla-industries-brp', 'hpLabelShort')
    } else {
      systemData.health.labelAbbr = game.i18n.localize('BRP.hp')
    }
    if (game.settings.get('sla-industries-brp', 'fpLabelLong')) {
      systemData.fatigue.label = game.settings.get('sla-industries-brp', 'fpLabelLong')
    } else {
      systemData.fatigue.label = game.i18n.localize('BRP.fatigue')
    }
    if (game.settings.get('sla-industries-brp', 'fpLabelShort')) {
      systemData.fatigue.labelAbbr = game.settings.get('sla-industries-brp', 'fpLabelShort')
    } else {
      systemData.fatigue.labelAbbr = game.i18n.localize('BRP.fp')
    }
    if (game.settings.get('sla-industries-brp', 'res5LabelLong')) {
      systemData.res5.label = game.settings.get('sla-industries-brp', 'res5LabelLong')
    } else {
      systemData.res5.label = game.i18n.localize('BRP.res5')
    }
    if (game.settings.get('sla-industries-brp', 'res5LabelShort')) {
      systemData.res5.labelAbbr = game.settings.get('sla-industries-brp', 'res5LabelShort')
    } else {
  ...
  static async create(data, options = {}) {
    ...
    if (game.settings.get('sla-industries-brp', "actorBRPID")) {
    ...
    if (actor.type === 'character' && game.settings.get('sla-industries-brp', 'useHPL')) {
      let brpid = "i.hit-location.general"

      const itemData = [{
        name: game.i18n.localize('BRP.general'),
        type: 'hit-location',
        img: 'systems/sla-industries-brp/assets/Icons/arm-bandage.svg',
    ...
    if (actor.type === 'character' && game.settings.get('sla-industries-brp', 'starterTraits') && game.settings.get('sla-industries-brp', 'usePersTrait')) {
      let newTraits = []
    ...
    if (actor.type === 'character') {
      ...
      // SLA baseline: every new character gets full SLA skill roster.
      await BRPActor.ensureSLASkillRoster(actor, {
  ...
  static async ensureSLASkillRoster(actor, { includeEbb = null, pruneEbbForNonEligible = true } = {}) {
    if (!actor || actor.type !== "character") return { created: 0, removed: 0, missing: [] }
    if (includeEbb === null || includeEbb === undefined) {
      includeEbb = SLAEbbSystem.isEbbSpecies(actor)
    }

    const seed = await SLASeedImporter.loadSeed("skills.json").catch(() => null)
    if (!seed?.skills?.length) {
      // Fallback for non-seeded worlds.
      if (game.settings.get('sla-industries-brp', 'starterSkills')) {
        let newSkills = []
        ...
        return { created: newSkills.length, removed: 0, missing: [] }
      }
      return { created: 0, removed: 0, missing: [] }
    }
    ...
    for (const item of actor.items.filter((i) => i.type === "skill")) {
      ...
      if (brpid) existing.set(brpid, item)
    }

    const createData = []
    const missing = []
    const iconUpdates = []
    for (const skillRow of seed.skills) {
      const skillName = String(skillRow?.name ?? "").trim()
      if (!skillName) continue
      const isEbbSkill = BRPActor._isEbbSkillName(skillName)
      if (isEbbSkill && !includeEbb) continue

      const brpid = await SLASeedImporter.resolveSkillBrpid(skillName)
      if (!brpid || brpid === "none") {
      ...
      const sourceDoc = (await game.system.api.brpid.fromBRPIDBest({ brpid }))[0]
      if (!sourceDoc || sourceDoc.type !== "skill") {
        missing.push(skillName)
        continue
      }
      ...
      if (!data.system.specialism && /\(\s*specialty\s*\)/i.test(skillName)) {
        const mainName = String(data.system.mainName || skillName.replace(/\(\s*specialty\s*\)/i, "").trim()).trim()
        data.system.specialism = true
        data.system.mainName = mainName
        data.system.specName = String(data.system.specName || "Specialty").trim()
        data.system.chosen = true
        data.name = `${mainName} (${data.system.specName})`
      }

      // Reserve one specialty slot when a skill uses specialties.
      if (data.system.specialism && !data.system.chosen) {
        const mainName = String(data.system.mainName || skillName).trim()
        const specName = String(data.system.specName || "Specialty").trim()
        data.system.mainName = mainName
        data.system.specName = specName
        data.system.chosen = true
        data.name = `${mainName} (${specName})`
      }

      const skillIcon = await SLASeedImporter.getSkillIconPath(skillName)
      if (skillIcon) {
        data.img = skillIcon
      }
    ...
    for (const skillItem of actor.items.filter((item) => item.type === "skill")) {
      const iconName = String(skillItem.system?.mainName || skillItem.name || "").trim()
      if (!iconName) continue
      const iconPath = await SLASeedImporter.getSkillIconPath(iconName)
      if (!iconPath || skillItem.img === iconPath) continue
      iconUpdates.push({ _id: skillItem.id, img: iconPath })
    }
    if (iconUpdates.length) {
      await actor.updateEmbeddedDocuments("Item", iconUpdates)
    }

    let removed = 0
    if (!includeEbb && pruneEbbForNonEligible) {
      const ebbSkillIds = actor.items
        .filter((item) => item.type === "skill")
        .filter((item) => BRPActor._isEbbSkillName(String(item.system?.mainName || item.name || "")))
        .map((item) => item.id)
      if (ebbSkillIds.length) {
        await actor.deleteEmbeddedDocuments("Item", ebbSkillIds)
        removed = ebbSkillIds.length
      }
    }

    if (createData.length || removed > 0) {
      await BRPActor.charBaseSkillScores(actor)
    }
    return { created: createData.length, removed, missing }
  }

  static async syncAllCharacterSkillRosters({ notify = true } = {}) {
