...
export class BRPactorItemDrop {
  ...
  static async _BRPonDropItemCreate(actor, itemData) {
    ...
    for (let nItm of itemData) {
      ...
      if (nItm.type != 'gear') {
        ...
        if (nItm.type === 'armour') {
          if (game.settings.get('sla-industries-brp', 'useHPL') && nItm.system.HPL) {
            reqResult = 0;
        ...
        if (nItm.type === 'power' && !game.settings.get('sla-industries-brp', [nItm.system.category])) {
          reqResult = 0;
        ...
        if (nItm.type === 'allegiance') {
          if (!game.settings.get('sla-industries-brp', 'useAlleg')) {
            reqResult = 0;
        ...
        if (nItm.type === 'reputation') {
          if (game.settings.get('sla-industries-brp', 'useReputation') === "0") {
            reqResult = 0;
            errMsg = nItm.name + " : " + game.i18n.localize('BRP.noRep');
          } else if (game.settings.get('sla-industries-brp', 'useReputation') === "1") {
        ...
        if (nItm.type === 'persTrait') {
          const isSlaTrait = Boolean(
            nItm.flags?.[game.system.id]?.slaTrait
            || nItm.flags?.brp?.slaTrait
            || String(nItm.flags?.[game.system.id]?.brpidFlag?.id ?? nItm.flags?.brp?.brpidFlag?.id ?? "").includes("i.persTrait.sla2-")
          );
          if (!game.settings.get('sla-industries-brp', 'usePersTrait') && !isSlaTrait) {
            reqResult = 0;
        ...
        if (nItm.type === 'passion') {
          if (!game.settings.get('sla-industries-brp', 'usePassion')) {
            reqResult = 0;
        ...
        if (nItm.type === 'psychic' && !SLAEbbSystem.isEbbSpecies(actor)) {
          reqResult = 0;
          errMsg = nItm.name + " : " + game.i18n.localize('BRP.ebbSpeciesOnly');
        }
        ...
        if (nItm.type === 'hit-location' && !game.settings.get('sla-industries-brp', 'useHPL')) {
  ...
  static async _calcBase(itm, actor) {
    ...
    if (itm.system.variable) {
      ...
      let newScore = 0
      if (stat1 != 'fixed') {
        if (stat1 != 'edu' || game.settings.get('sla-industries-brp', 'useEDU')) {
      ...
      if (stat2 != 'fixed') {
        if (stat2 != 'edu' || game.settings.get('sla-industries-brp', 'useEDU')) {
  ...
  static async hitLocationDialog(actor) {
    ...
    const html = await foundry.applications.handlebars.renderTemplate('systems/sla-industries-brp/templates/dialog/hitLocChoice.html', data)
  ...
  static async _dropPersonality(itm, actor) {
    ...
    if (itm.type === 'profession') {
      for (let nPwr of (itm.system.powers ?? [])) {
        ...
        if (!game.settings.get('sla-industries-brp', [newSkill.system.category])) {
    ...
    const primarySkills = Array.isArray(itm.system.skills) ? [...itm.system.skills] : []
    if (itm.type === 'culture') {
      const speciesRows = await this._speciesFlagSkillRows(itm)
      for (const row of speciesRows) {
        const existing = primarySkills.find((entry) => entry.brpid === row.brpid)
        if (existing) {
          existing.bonus = Number(existing.bonus ?? 0) + Number(row.bonus ?? 0)
        } else {
          primarySkills.push(row)
        }
      }
    }

    //Get each skill etc on the main list
    for (let nItm of primarySkills) {
    ...
    for (let nItm of skillList) {
      ...
      const newItemBrpid = nItm.flags?.[game.system.id]?.brpidFlag?.id ?? nItm.flags?.brp?.brpidFlag?.id
      let actItem = (await actor.items
        .filter(dItm => (dItm.flags?.[game.system.id]?.brpidFlag?.id ?? dItm.flags?.brp?.brpidFlag?.id))
        .filter(cItm => (cItm.flags?.[game.system.id]?.brpidFlag?.id ?? cItm.flags?.brp?.brpidFlag?.id) === newItemBrpid))[0]
      if (actItem) {
        if (itm.type === 'personality') {
    ...
    if (itm.type === 'culture') {
      ...
      changes = Object.assign(changes, {
        'system.move': Number(itm.system.move),
        'system.culture': itm.name
      })
      await actor.update(changes)
      await this._setSpeciesChoiceFlags(itm, actor)
      if (game.brp?.BRPActor?.ensureSLASkillRoster) {
        await game.brp.BRPActor.ensureSLASkillRoster(actor, {
  ...
  static async cultureDelete(event, actor) {
    ...
    await actor.unsetFlag(game.system.id, "slaPendingSkillChoices")
    if (game.brp?.BRPActor?.ensureSLASkillRoster) {
      await game.brp.BRPActor.ensureSLASkillRoster(actor, {
        includeEbb: false,
        pruneEbbForNonEligible: true
  ...
  static async _speciesFlagSkillRows(itm) {
    const scope = game.system.id
    const speciesMeta = itm?.flags?.[scope]?.slaSpecies ?? itm?.flags?.brp?.slaSpecies ?? {}
    const bonuses = Array.isArray(speciesMeta?.skillBonuses) ? speciesMeta.skillBonuses : []
    ...
    for (const row of bonuses) {
      ...
      const brpid = await SLASeedImporter.resolveSkillBrpid(skillRef)
  ...
  static async _setSpeciesChoiceFlags(itm, actor) {
    const scope = game.system.id
    const speciesMeta = itm?.flags?.[scope]?.slaSpecies ?? itm?.flags?.brp?.slaSpecies ?? {}
    const choices = Array.isArray(speciesMeta?.choiceBonuses) ? speciesMeta.choiceBonuses : []
    if (!choices.length) {
      await actor.unsetFlag(scope, "slaPendingSkillChoices")
      return
    }

    await actor.setFlag(scope, "slaPendingSkillChoices", choices)
    ...
    if (totalChoices > 0) {
      ui.notifications.info(
        `${actor.name}: ${itm.name} has ${totalChoices} unassigned species skill choice bonus(es).`
      )
    }
  }
  ...
  static async selectFromRadio(list, title) {
    ...
    if (Object.keys(newList).length === 1) {
      ...
      let destination = 'systems/sla-industries-brp/templates/dialog/selectItem.html';
      let data = {
        headTitle: title,
        newList,
      }
