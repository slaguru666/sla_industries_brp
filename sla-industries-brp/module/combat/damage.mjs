...
export class BRPDamage {
  ...
  static async addDamage(event, actor, token, damage) {
    ...
    if (!locationId && game.settings.get('sla-industries-brp', 'useHPL')) {
    ...
    return this.applyResolvedDamage({ actor: partic, damage, locationId })
  }

  static async applyResolvedDamage({ actor = null, token = null, damage = 0, locationId = "" } = {}) {
    const partic = actor ?? await BRPactorDetails._getParticipantPriority(token, actor)
    if (!partic) {
      ui.notifications.warn(game.i18n.localize('BRP.slaDamageNoTarget'))
    ...
    try {
      ...
      const mitigated = await SLADrugSystem.modifyIncomingDamage({
        actor: partic,
        damage,
        source: "manual"
      })
      damage = Number(mitigated.damage ?? damage)
    } catch (err) {
      console.warn("sla-industries-brp | Could not apply drug damage mitigation", err)
    }
    ...
    if (game.settings.get('sla-industries-brp', 'useHPL')) {
      location = partic.items.get(locationId)
      if (!location) {
        const hitLocs = partic.items.filter(itm => itm.type === 'hit-location')
        if (hitLocs.length === 1) {
          location = hitLocs[0]
          locationId = location.id
        } else {
          location = hitLocs.find(loc => loc.system.locType === 'general') ?? hitLocs[0]
          locationId = location?.id ?? ""
        }
      }
      if (!location) {
    ...
    const result = await this._applyDamageToParticipant(partic, damage, locationId, location)
    return result
  }

  static async _applyDamageToParticipant(partic, damage, locationId, location = null) {
    ...
    if (game.settings.get('sla-industries-brp', 'useHPL') && location.system.locType != 'general') {
    ...
    } else if (!game.settings.get('sla-industries-brp', 'useHPL')) {
    ...
    if (game.settings.get('sla-industries-brp', 'useHPL')) { await location.update(checkprop) }
  ...
  static async healingAmount(title, getType, getWnd, wndList, healTypes) {
    const html = await foundry.applications.handlebars.renderTemplate(
      'systems/sla-industries-brp/templates/dialog/treatWound.html',
      {
        getType,
        getWnd,
        wndList,
  ...
  static async getWoundForm(getDam, getLoc, name, locs) {
    ...
    const html = await foundry.applications.handlebars.renderTemplate(
      'systems/sla-industries-brp/templates/dialog/newWound.html',
      {
        getDam,
        getLoc,
        locs
