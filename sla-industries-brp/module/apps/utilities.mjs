...
export class BRPUtilities {
  ...
  static async beastiaryMode(toggle) {
    let state = await game.settings.get('sla-industries-brp', 'beastiary')
    await game.settings.set('sla-industries-brp', 'beastiary', !state)
  ...
  static async sendToChat(el, actor, dataitem) {
    ...
    let chatTemplate = 'systems/sla-industries-brp/templates/chat/itemDescription.html'
