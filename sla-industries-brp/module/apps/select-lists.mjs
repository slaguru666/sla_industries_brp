export class BRPSelectLists {
  static _brpid(item) {
    return item?.flags?.[game.system.id]?.brpidFlag?.id ?? item?.flags?.brp?.brpidFlag?.id ?? "";
  }
  ...
  static async addStatOptions(characteristic) {
    ...
    if (game.settings.get('sla-industries-brp', 'useEDU')) {
  ...
  static async getCategoryOptions() {
    ...
    for (let itm of skillCatList) {
      const brpid = this._brpid(itm);
      if (brpid) {
        options = Object.assign(options, { [brpid]: itm.name })
      }
    }
  ...
  static getHitLocOptions(actor) {
    ...
    for (let i of actor.items) {
      if (i.type === 'hit-location') {
        if (game.settings.get('sla-industries-brp', 'useHPL') && i.system.locType != 'general') {
          newOption = { [i.id]: i.system.displayName, };
          options = Object.assign(options, newOption)
        } else if (!game.settings.get('sla-industries-brp', 'useHPL') && i.system.locType === 'general') {
  ...
  static async getWeaponSkillOptions(skillId) {
    ...
    let newList = skillList
      .filter(itm => (itm.system.combat || itm.system.category === 'i.skillcat.combat'))
      .map(itm => { return { brpid: this._brpid(itm), name: itm.name } })
  ...
  static async getDifficultyOptions() {
    let options = {
      "advantage": game.i18n.localize("BRP.adv") + " (1D100 + tens re-roll, keep best)",
      "normal": game.i18n.localize("BRP.normal") + " (1D100)",
      "disadvantage": game.i18n.localize("BRP.dis") + " (1D100 + tens re-roll, keep worst)",
    };
    return options;
  }
  ...
  static async getCHDifficultyOptions() {
    return BRPSelectLists.getDifficultyOptions();
  }
