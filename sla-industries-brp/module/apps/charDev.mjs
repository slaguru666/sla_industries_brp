...
export class BRPCharDev {
  ...
  static async developmentPhase(toggle) {
    let state = await game.settings.get('sla-industries-brp', 'development')
    await game.settings.set('sla-industries-brp', 'development', !state)
  ...
  static async xpCheck(actor, token, itemId, type, opp) {
    ...
    if (result.level === 1) {
      if (type === 'fixed') {
        improvVal = game.settings.get('sla-industries-brp', 'xpFixed')
      } else {
        let roll = new Roll(game.settings.get('sla-industries-brp', 'xpFormula'))
        await roll.evaluate();
  ...
  static async xpRoll(target, bonus) {
    ...
    if (game.settings.get('sla-industries-brp', 'xpRollDice') && game.modules.get('dice-so-nice')?.active) {
  ...
  static async xpChatCard(success, actorName, actrImg) {
    ...
    const messageTemplate = 'systems/sla-industries-brp/templates/chat/XP-result.html'
