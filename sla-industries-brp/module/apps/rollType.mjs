...
export class BRPRollType {
  ...
  static async _onStatRoll(event) {
    ...
    let characteristic = dataset.characteristic;
    const derivedTarget = Number(dataset.target ?? NaN);
    const isDerived = String(dataset.type ?? "").toLowerCase() === "derived" && Number.isFinite(derivedTarget);
    const derivedLabel = String(dataset.label ?? "").trim();
    let label = derivedLabel || "";
    let targetScore = undefined;
    let rawScore = undefined;
    if (isDerived) {
      targetScore = derivedTarget;
      rawScore = Math.round(derivedTarget / 5);
    }
  ...
  static async _onWeaponRoll(event) {
    ...
    let cardType = 'NO'
    let fireMode = "single"
    if (event.altKey) fireMode = "burst"
    if (event.ctrlKey || event.metaKey) fireMode = "auto"
    if (game.settings.get('sla-industries-brp', 'quickCombat') && event.shiftKey) {
      rollType = 'QC'
      cardType = "NO"
      event.shiftKey = false
    } else if (game.settings.get('sla-industries-brp', 'switchShift')) {
      event.shiftKey = !event.shiftKey
    }
    BRPCheck._trigger({
      rollType,
