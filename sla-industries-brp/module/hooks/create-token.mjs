...
export function listen() {
  Hooks.on('createToken', async (tokenDocument, options, creatorId) => {
    ...
    if (tokenDocument.actor.type === 'npc') {
      if (game.settings.get('sla-industries-brp', 'npcName')) {
        let updates = [({ _id: tokenDocument._id, displayName: CONST.TOKEN_DISPLAY_MODES[game.settings.get('sla-industries-brp', 'npcName')] })]
        canvas.scene.updateEmbeddedDocuments("Token", updates)
      }
      if (game.settings.get('sla-industries-brp', 'npcBars')) {
        let updates = [({ _id: tokenDocument._id, displayBars: CONST.TOKEN_DISPLAY_MODES[game.settings.get('sla-industries-brp', 'npcBars')] })]
        canvas.scene.updateEmbeddedDocuments("Token", updates)
      }
    }
    ...
    if (tokenDocument._object.actor.type !== 'character' && (tokenDocument._object.actor.hasRollableCharacteristics) && !tokenDocument._object.actor.prototypeToken.actorLink) {
      switch (game.settings.get('sla-industries-brp', 'tokenDropMode')) {
        case 'ask':
          new Dialog(
            {
