...
export class BRPSkillSheet extends foundry.appv1.sheets.ItemSheet {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      ...
      template: 'systems/sla-industries-brp/templates/item/skill.html',
      width: 520,
      height: 620,
  ...
  async getData() {
    ...
    sheetData.slaTraitPreview = {
      flatMod: 0,
      flatSigned: "+0",
      effects: [],
      summary: "",
      effectiveTotal: Number(itemData.system.total ?? 0),
      has: false
    };

    if (sheetData.hasOwner && this.item.parent?.type === "character") {
      const preview = await SLATraitEngine.getCheckContext(this.item.parent, {
        rollType: "SK",
        cardType: "NO",
        skillId: this.item.id,
        label: this.item.name,
        reason: "skill-sheet-preview"
      }, { preview: true });

      const flatMod = Number(preview?.flatMod ?? 0);
      sheetData.slaTraitPreview = {
        flatMod,
        flatSigned: `${flatMod >= 0 ? "+" : ""}${flatMod}`,
        effects: Array.isArray(preview?.effects) ? preview.effects : [],
        summary: String(preview?.summary ?? ""),
        effectiveTotal: Number(itemData.system.total ?? 0) + flatMod,
        has: flatMod !== 0 || (Array.isArray(preview?.effects) && preview.effects.length > 0)
      };
    }
