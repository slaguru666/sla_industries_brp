...
export class BRPID {
  static init() {
    const addIndexField = (collection, field) => {
      if (!collection.includes(field)) collection.push(field)
    }
    const currentScopeField = `flags.${game.system.id}.brpidFlag`
    const legacyScopeField = 'flags.brp.brpidFlag'
    addIndexField(CONFIG.Actor.compendiumIndexFields, currentScopeField)
    addIndexField(CONFIG.Actor.compendiumIndexFields, legacyScopeField)
    addIndexField(CONFIG.Item.compendiumIndexFields, currentScopeField)
    addIndexField(CONFIG.Item.compendiumIndexFields, legacyScopeField)
    addIndexField(CONFIG.JournalEntry.compendiumIndexFields, currentScopeField)
    addIndexField(CONFIG.JournalEntry.compendiumIndexFields, legacyScopeField)
    addIndexField(CONFIG.RollTable.compendiumIndexFields, currentScopeField)
    addIndexField(CONFIG.RollTable.compendiumIndexFields, legacyScopeField)
    game.system.api = {
      ...(game.system.api ?? {}),
      brpid: BRPID
    }
  ...
  static guessGroupFromDocument(document) {
    return BRPID.guessGroupFromKey(BRPID.getBRPIDFlag(document)?.id)
  }
  ...
  static async expandItemArray({ itemList, lang = game.i18n.lang, langFallback = true, showLoading = false } = {}) {
    ...
    if (brpids.length) {
      ...
      for (const brpid of brpids) {
        const item = found.find(i => BRPID.getBRPIDFlag(i)?.id === brpid)
        if (item) {
          all.push(item)
        }
      }
      if (all.length < brpids.length) {
        ...
        for (const doc of all) {
          notmissing.push(BRPID.getBRPIDFlag(doc)?.id)
        }
  ...
  static findBRPIdInList(brpid, list) {
    ...
    return (typeof list.filter === 'undefined' ? Object.values(list) : list).filter(i => BRPID.getBRPIDFlag(i)?.id === brpid || (itemName !== '' && itemName === i.name))
  }
  ...
  static filterBestBRPID(documents) {
    ...
    for (const doc of documents) {
      const docFlag = BRPID.getBRPIDFlag(doc)
      const docBRPID = docFlag?.id
      if (docBRPID) {
        ...
        let docPriority = parseInt(docFlag?.priority ?? Number.MIN_SAFE_INTEGER, 10)
        docPriority = isNaN(docPriority) ? Number.MIN_SAFE_INTEGER : docPriority
        let existingPriority = parseInt(BRPID.getBRPIDFlag(currentDoc)?.priority ?? Number.MIN_SAFE_INTEGER, 10)
  ...
  static filterAllBRPID(documents, langFallback) {
    ...
    for (const doc of documents) {
      ...
      if (docBRPID) {
        ...
        const docLang = docFlag?.lang ?? 'en'
        const existingLang = BRPID.getBRPIDFlag(currentDoc)?.lang ?? 'en'
    ...
    for (const pack of game.packs) {
      ...
      if (pack.documentName === documentType) {
        ...
        const indexInstances = pack.index.filter((i) => {
          const brpidFlag = BRPID.getBRPIDFlag(i)
          if (typeof brpidFlag === 'undefined') {
            return false
          }
          return brpidRegExp.test(brpidFlag.id) && [lang, (langFallback ? 'en' : '-')].includes(brpidFlag.lang)
  ...
  static compareBRPIDPrio(a, b) {
    const aPrio = parseInt(BRPID.getBRPIDFlag(a)?.priority ?? Number.MIN_SAFE_INTEGER, 10)
    const bPrio = parseInt(BRPID.getBRPIDFlag(b)?.priority ?? Number.MIN_SAFE_INTEGER, 10)
    return (isNaN(bPrio) ? Number.MIN_SAFE_INTEGER : bPrio) - (isNaN(aPrio) ? Number.MIN_SAFE_INTEGER : aPrio)
  }
